CREATE SCHEMA tSQLt_testutil;
GO
CREATE SCHEMA ApplyConstraintTests;
GO
CREATE SCHEMA ApplyTriggerTests;
GO
CREATE SCHEMA AssertEmptyTableTests;
GO
CREATE SCHEMA AsertEqualsStringTests;
GO
CREATE SCHEMA AssertEqualsTableSchemaTests;
GO
CREATE SCHEMA AssertEqualsTableTests;
GO
CREATE SCHEMA AssertEqualsTests;
GO
CREATE SCHEMA AssertLikeTests;
GO
CREATE SCHEMA AssertNotEqualsTests;
GO
CREATE SCHEMA AssertObjectDoesNotExistTests;
GO
CREATE SCHEMA AssertObjectExistsTests;
GO
CREATE SCHEMA tSQLt_test_AssertResultSetsHaveSameMetaData;
GO
CREATE SCHEMA AssertStringInTests;
GO
CREATE SCHEMA ExpectExceptionTests;
GO
CREATE SCHEMA ExpectNoExceptionTests;
GO
CREATE SCHEMA FailTests;
GO
CREATE SCHEMA FakeFunctionTests;
GO
CREATE SCHEMA FakeTableTests;
GO
CREATE SCHEMA RemoveObjectTests;
GO
CREATE SCHEMA RemoveObjectIfExistsTests;
GO
CREATE SCHEMA SpyProcedureTests;
GO