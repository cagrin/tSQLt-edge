...
CREATE PROCEDURE Test_XmlResultFormatter.ValidateXml
    @Input XML
AS
BEGIN
    DECLARE @Command NVARCHAR(MAX) =
'CREATE XML SCHEMA COLLECTION tSQLt.JUnitSchema
...
Permission to waive conditions of this license may be requested from Windy Road Support (http://windyroad.org/support).</xs:documentation>
    ...
    EXEC sp_executesql N'DECLARE @Result XML(tSQLt.JUnitSchema) = @Input', N'@Input XML', @Input;
END;
GO
...
CREATE PROCEDURE Test_XmlResultFormatter.Test_XmlResultIsValid
AS
BEGIN
    CREATE TABLE #XmlResult (Result XML)
    INSERT INTO #XmlResult
    ...
    EXEC Test_XmlResultFormatter.ValidateXml @Result;
